...
export class Homestep4Component implements OnInit{
  ...
  changeTexts(){
    let texts: any[] = [
      ...
      "Todos los datos en Aurora Studios son confidenciales y están cifrados de punta a punta con un cifrado especial.",
      "¿Sabias que...? Aurora HomeServices®️ WebKit es nuestro servicio general en nuestros servidores, controla el trafico y la seguridad de nuestros usuarios.",
      "Todas las solicitudes de registro son revisadas por nuestro equipo de seguridad, para garantizar ventas seguras y confiables.",
    ]

    const random = Math.floor(Math.random() * texts.length);

    this.texts = texts[random];
  }
