...
export class ProfileComponent implements OnInit{
  ...
  transformData(dateServer: string){
    ...
    this.message = message + ' en la plataforma';
  }
  ...
  getMessage(verification: number):string{
    switch(verification){
      case 1:
        return 'Vendedor seguro';
      case 2:
        return 'Vendedor oficial en HomeServices®️';
      default:
        return 'Vendedor no verificado';
    }
  }
  ...
  getStylesMessage(verification: number):string{
    switch(verification){
      case 1:
        return 'mb-1 text-dark';
      case 2:
        return 'mb-1 text-success';
      default:
        return 'mb-1 text-danger';
    }
  }
  ...
  getReviews(): Promise<boolean>{
    return new Promise((resolve, reject) => {
      Notiflix.Loading.dots('Descargando paquetes...',{
        clickToClose: false,
        svgColor: '#a95eff',
        className: 'font-b',
        backgroundColor: '#fff',
      ...
      this._API_.getReviews_Profile(packet).subscribe((result: any) => {
        if(result.get === true){
          if(result.data.length > 0){
            ...
            Notiflix.Loading.remove();
            this.reviews = [];
            this.alertA = 'No hay reseñas para este usuario. Tenga precaución al realizar transacciones.'
            resolve(true)
          }
        }
        else{
