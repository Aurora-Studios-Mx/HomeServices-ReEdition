...
export class NavbarComponent implements OnInit{
  ...
  getAllinformation(): Promise<boolean>{
    return new Promise((resolve, reject) => {
      this._serv.getNavContent().subscribe((result: any) => {
        ...
        console.log(error);
        Notiflix.Notify.warning('El motor de búsqueda no mostrará sugerencias.', {
  ...
  async ngOnInit(){
    ...
    if(this.extended === false){
      this.somebodyClass = 'navbar navbar-expand-lg p-2 user-select-none z-1 dropped-shadow-tiny';
    }
